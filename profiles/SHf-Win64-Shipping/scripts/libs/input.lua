...
-- SHf/UE5.7 can expose pawn classes without the standard APawn controller-rotation
-- reflected properties. Direct reads/writes throw LuaVR set_property errors, so keep
-- these optional and never let a missing property break the per-frame input tick.
local function safeGetObjectProperty(obj, prop, default)
...
local function safeSetObjectProperty(obj, prop, value)
...
local function resetPawnSettings()
	...
	if uevrUtils.getValid(pawn) ~= nil and pawnSettings ~= nil then
		...
		if pawnSettings.hasControllerRotationPitch then safeSetObjectProperty(pawn, "bUseControllerRotationPitch", pawnSettings.bUseControllerRotationPitch) end
		if pawnSettings.hasControllerRotationYaw then safeSetObjectProperty(pawn, "bUseControllerRotationYaw", pawnSettings.bUseControllerRotationYaw) end
		if pawnSettings.hasControllerRotationRoll then safeSetObjectProperty(pawn, "bUseControllerRotationRoll", pawnSettings.bUseControllerRotationRoll) end
		local movement = safeGetObjectProperty(pawn, "CharacterMovement", nil)
		if movement ~= nil then
			if pawnSettings.hasOrientRotationToMovement then safeSetObjectProperty(movement, "bOrientRotationToMovement", pawnSettings.bOrientRotationToMovement) end
			if pawnSettings.hasUseControllerDesiredRotation then safeSetObjectProperty(movement, "bUseControllerDesiredRotation", pawnSettings.bUseControllerDesiredRotation) end
		end
		pawnSettings = nil
	end
end
...
local function updatePawnSettings()
	if pawn ~= nil then
		...
		if pawnSettings == nil then
			pawnSettings = {}
			pawnSettings.bUseControllerRotationPitch = safeGetObjectProperty(pawn, "bUseControllerRotationPitch", nil)
			pawnSettings.bUseControllerRotationYaw = safeGetObjectProperty(pawn, "bUseControllerRotationYaw", nil)
			pawnSettings.bUseControllerRotationRoll = safeGetObjectProperty(pawn, "bUseControllerRotationRoll", nil)
			pawnSettings.hasControllerRotationPitch = pawnSettings.bUseControllerRotationPitch ~= nil
			pawnSettings.hasControllerRotationYaw = pawnSettings.bUseControllerRotationYaw ~= nil
			pawnSettings.hasControllerRotationRoll = pawnSettings.bUseControllerRotationRoll ~= nil
			pawnSettings.bOrientRotationToMovement = safeGetObjectProperty(movement, "bOrientRotationToMovement", nil)
			pawnSettings.bUseControllerDesiredRotation = safeGetObjectProperty(movement, "bUseControllerDesiredRotation", nil)
			pawnSettings.hasOrientRotationToMovement = pawnSettings.bOrientRotationToMovement ~= nil
			pawnSettings.hasUseControllerDesiredRotation = pawnSettings.bUseControllerDesiredRotation ~= nil
		end

		local useControllerRotationPitch = getParameter("useControllerRotationPitch")
		if pawnSettings.hasControllerRotationPitch then
			if useControllerRotationPitch ~= nil and useControllerRotationPitch ~= ETriState.DEFAULT then
				safeSetObjectProperty(pawn, "bUseControllerRotationPitch", useControllerRotationPitch == ETriState.TRUE)
			else
				safeSetObjectProperty(pawn, "bUseControllerRotationPitch", pawnSettings.bUseControllerRotationPitch)
			end
		end
		local useControllerRotationYaw = getParameter("useControllerRotationYaw")
		if pawnSettings.hasControllerRotationYaw then
			if useControllerRotationYaw ~= nil and useControllerRotationYaw ~= ETriState.DEFAULT then
				safeSetObjectProperty(pawn, "bUseControllerRotationYaw", useControllerRotationYaw == ETriState.TRUE)
			else
				safeSetObjectProperty(pawn, "bUseControllerRotationYaw", pawnSettings.bUseControllerRotationYaw)
			end
		end
		local useControllerRotationRoll = getParameter("useControllerRotationRoll")
		if pawnSettings.hasControllerRotationRoll then
			if useControllerRotationRoll ~= nil and useControllerRotationRoll ~= ETriState.DEFAULT then
				safeSetObjectProperty(pawn, "bUseControllerRotationRoll", useControllerRotationRoll == ETriState.TRUE)
			else
				safeSetObjectProperty(pawn, "bUseControllerRotationRoll", pawnSettings.bUseControllerRotationRoll)
			end
		end

		if movement ~= nil then
			local orientRotationToMovement = getParameter("orientRotationToMovement")
			if pawnSettings.hasOrientRotationToMovement then
				if orientRotationToMovement ~= nil and orientRotationToMovement ~= ETriState.DEFAULT then
					safeSetObjectProperty(movement, "bOrientRotationToMovement", orientRotationToMovement == ETriState.TRUE)
				else
					safeSetObjectProperty(movement, "bOrientRotationToMovement", pawnSettings.bOrientRotationToMovement)
				end
			end
			local useControllerDesiredRotation = getParameter("useControllerDesiredRotation")
			if pawnSettings.hasUseControllerDesiredRotation then
				if useControllerDesiredRotation ~= nil and useControllerDesiredRotation ~= ETriState.DEFAULT then
					safeSetObjectProperty(movement, "bUseControllerDesiredRotation", useControllerDesiredRotation == ETriState.TRUE)
				else
					safeSetObjectProperty(movement, "bUseControllerDesiredRotation", pawnSettings.bUseControllerDesiredRotation)
				end
			end
		end
...
uevr.sdk.callbacks.on_xinput_get_state(function(retval, user_index, state)
	...
	if not isDisabled then --and pawnRotationMode ~= M.PawnRotationMode.NONE then
		...
		if yawChange ~= 0 and decoupledYaw~= nil and rootComponent ~= nil and getParameter("pawnRotationModeDisableRotation") ~= true then
			rootComponent:K2_SetWorldRotation(uevrUtils.rotator(0,decoupledYaw+bodyRotationOffset,0),false,reusable_hit_result,false)
		end
	end
end)
